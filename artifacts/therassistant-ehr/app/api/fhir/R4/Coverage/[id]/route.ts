...
import { COVERAGE_DB_COLUMNS, intakeRowToCoverage, type IntakeSubmissionRow } from "@/lib/fhir/coverage";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!id || !isUuid(id)) return operationOutcome("error", "not-found", `Coverage/${id} not found`, 404);

    const { data, error } = await supabase
      .from("intake_submissions")
      .select(COVERAGE_DB_COLUMNS)
      .eq("id", id)
      .eq("organization_id", auth.organizationId)
      .maybeSingle();
    if (error) return operationOutcome("error", "exception", error.message, 500);
    if (!data) return operationOutcome("error", "not-found", `Coverage/${id} not found`, 404);

    return fhirJson(intakeRowToCoverage(data as IntakeSubmissionRow, baseUrlOf(request)));
  } catch (err) {

...
import { OBSERVATION_DB_COLUMNS, checkInRowToObservation, type CheckInRow } from "@/lib/fhir/observation";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!id || !isUuid(id)) return operationOutcome("error", "not-found", `Observation/${id} not found`, 404);

    const { data, error } = await supabase
      .from("patient_check_ins")
      .select(OBSERVATION_DB_COLUMNS)
      .eq("id", id)
      .eq("organization_id", auth.organizationId)
      .maybeSingle();
    if (error) return operationOutcome("error", "exception", error.message, 500);
    if (!data) return operationOutcome("error", "not-found", `Observation/${id} not found`, 404);

    return fhirJson(checkInRowToObservation(data as CheckInRow, baseUrlOf(request)));
  } catch (err) {

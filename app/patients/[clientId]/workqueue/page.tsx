...
export default function ClientWorkqueuePage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/patients/${clientId}/workqueue?organizationId=${encodeURIComponent(orgId)}`, { cache: "no-store" })
      .then((r) => r.json() as Promise<{ success: boolean; items?: WorkItem[]; error?: string }>)
      .then((json) => {

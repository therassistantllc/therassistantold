...
export default function VisitsAppointmentsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/patients/${clientId}/appointments?organizationId=${encodeURIComponent(orgId)}`, { cache: "no-store" })
      .then((r) => r.json() as Promise<{ success: boolean; appointments?: Appointment[]; error?: string }>)
      .then((json) => {
